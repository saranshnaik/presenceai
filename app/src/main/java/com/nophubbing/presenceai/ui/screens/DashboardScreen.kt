    ...
    Box(Modifier.fillMaxSize().background(
        ...
        Column(Modifier.fillMaxSize()) {
            ...
            when (tab) {
                ...
                2 -> InsightsScreen(vm)
                3 -> SignalsTab(featureVals, pDrift, pPhub)
                4 -> DebugTab(pDrift, pPhub, presenceScore, updateCount, banditStats, featureVals, isRunning)
            }
        }
    }
}
    ...
    Row(Modifier.fillMaxWidth().padding(horizontal = 20.dp, vertical = 14.dp),
        ...
        Column {
            Text("Presence AI", fontSize = 24.sp, fontWeight = FontWeight.Black,
                color = PresenceColors.TextPrimary, letterSpacing = (-0.5).sp)
            Row(verticalAlignment = Alignment.CenterVertically) {
                if (isRunning) {
                    val pulse = rememberInfiniteTransition(label = "hp")
                    val a by pulse.animateFloat(0.4f, 1f,
                        infiniteRepeatable(tween(900), RepeatMode.Reverse), label = "ha")
                    Box(Modifier.size(6.dp).background(PresenceColors.AccentGreen.copy(a), CircleShape))
                } else {
                    Box(Modifier.size(6.dp).background(PresenceColors.TextDim, CircleShape))
                }
                Spacer(Modifier.width(5.dp))
                Text(if (isRunning) "Observing gracefully" else "Monitoring paused",
                    color = if (isRunning) PresenceColors.AccentGreen else PresenceColors.TextMuted,
                    fontSize = 12.sp)
            }
        }
        Row(
            Modifier.clip(RoundedCornerShape(20.dp))
                .background(if (isRunning) Color(0xFF0d2a1a) else Color(0xFF1a0d1a))
                .border(1.dp,
                    if (isRunning) Color(0xFF1a4a2e) else Color(0xFF3a1a2e),
                    RoundedCornerShape(20.dp))
                .clickable(onClick = onToggle).padding(horizontal = 16.dp, vertical = 9.dp),
            verticalAlignment = Alignment.CenterVertically,
            horizontalArrangement = Arrangement.spacedBy(6.dp)
        ) {
            Box(Modifier.size(7.dp).clip(CircleShape)
                .background(if (isRunning) PresenceColors.AccentGreen else PresencePink))
            Text(if (isRunning) "ON" else "OFF",
                color = if (isRunning) PresenceColors.AccentGreen else PresencePink,
...
private fun ScoreRing(presenceScore: Float, pPhub: Float, isLive: Boolean) {
    ...
    Column(Modifier.fillMaxWidth(), horizontalAlignment = Alignment.CenterHorizontally) {
        ...
        if (isLive) {
            ...
            Text("Monitoring paused", color = PresenceColors.TextMuted, fontSize = 10.sp)
        }
    }
}
...
private fun insightText(pPhub: Float, score: Float, unlocks: Int, micro: Int) = when {
    ...
    unlocks > 6 && micro > 3 -> "You've unlocked $unlocks times with $micro quick checks. Try placing the phone face-down."
